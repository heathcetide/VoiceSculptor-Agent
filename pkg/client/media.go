...
type AudioStream struct {
	...
	Timestamp uint32 // Timestamp for the audio data
}
...
func NewMediaHandler(ctx context.Context, logger *logrus.Logger) (*MediaHandler, error) {
	...
	voiceInputChannel := make(chan bool, 1)
	return &MediaHandler{
		ctx:               ctx,
		cancel:            cancel,
		logger:            logger,
		...
		voiceInputChannel: voiceInputChannel,
	}, nil
}
...
func (mh *MediaHandler) Setup(codec string, iceServers []webrtc.ICEServer) (string, error) {
	...
	peerConnection.OnConnectionStateChange(func(state webrtc.PeerConnectionState) {
		...
		if state == webrtc.PeerConnectionStateConnected {
			mh.connected = true
			err := mh.initPlaybackDevice(codec)
			if err != nil {
				fmt.Printf("Failed to start audio capture: %v", err)
				mh.logger.Errorf("Failed to start audio capture: %v", err)
			}
			err = mh.startAudioCapture(codec)
			if err != nil {
				fmt.Printf("Failed to start audio capture: %v", err)
				mh.logger.Errorf("Failed to start audio capture: %v", err)
			}
...
// HandleAudioStream processes incoming audio streams and resets the voice input timer
func (mh *MediaHandler) HandleAudioStream(stream AudioStream) {
	// Reset the voice input timer every time we receive audio stream data
	select {
	case mh.voiceInputChannel <- true: // Reset the timer by signaling input
	default:
	}

	// Optionally, handle the stream data, for example, appending to buffer or processing it
	mh.logger.Infof("Received audio stream with timestamp: %v", stream.Timestamp)

	// You can store or process the audio data here, for example:
	mh.bufferMutex.Lock()
	mh.buffer = append(mh.buffer, stream.Data...)
	mh.bufferMutex.Unlock()

	// Additional processing based on the stream's data can be added here
}
...
func (mh *MediaHandler) startAudioCapture(codec string) error {
	...
	captureDevice, err := malgo.InitDevice(mh.playbackCtx.Context, deviceConfig, malgo.DeviceCallbacks{
		...
		Data: func(outputSamples, inputSamples []byte, frameCount uint32) {
			if !mh.connected {
				mh.logger.Warn("Not connected yet, skipping input samples")
				return
			}
			mh.bufferMutex.Lock()
	...
	// 检查捕获设备初始化是否成功
	if err != nil {
		mh.logger.Errorf("Failed to initialize capture device: %v", err)
		return fmt.Errorf("failed to initialize capture device: %v", err)
	}
	mh.captureDevice = captureDevice
	mh.logger.Info("Capture device initialized successfully")

	// 启动捕获设备
	err = captureDevice.Start()
	if err != nil {
		mh.logger.Errorf("Failed to start capture device: %v", err)
		return fmt.Errorf("failed to start capture device: %v", err)
	}
	mh.logger.Info("Capture device started successfully")
