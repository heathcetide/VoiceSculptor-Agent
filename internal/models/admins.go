...
type AdminObject struct {
	...
	Orders      []voiceSculptor.Order `json:"orders"`              // Default orders of the object
	...
	GetDB            voiceSculptor.GetDB            `json:"-"`
	BeforeCreate     voiceSculptor.BeforeCreateFunc `json:"-"`
	BeforeRender     voiceSculptor.BeforeRenderFunc `json:"-"`
	BeforeUpdate     voiceSculptor.BeforeUpdateFunc `json:"-"`
	BeforeDelete     voiceSculptor.BeforeDeleteFunc `json:"-"`
...
func GetHibiscusAdminObjects() []AdminObject {
	...
	iconUser, _ := voiceSculptor.EmbedStaticAssets.ReadFile("static/img/icon_user.svg")
	iconGroup, _ := voiceSculptor.EmbedStaticAssets.ReadFile("static/img/icon_group.svg")
	iconMembers, _ := voiceSculptor.EmbedStaticAssets.ReadFile("static/img/icon_members.svg")
	iconConfig, _ := voiceSculptor.EmbedStaticAssets.ReadFile("static/img/icon_config.svg")
	...
	return []AdminObject{
		{
			...
			Orders:      []voiceSculptor.Order{{"UpdatedAt", voiceSculptor.OrderOpDesc}},
...
func WithAdminAuth() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		if user == nil {
			...
			signUrl := util.GetValue(db, constants.KEY_SITE_SIGNIN_URL)
			if signUrl == "" {
				voiceSculptor.AbortWithJSONError(ctx, http.StatusUnauthorized, errors.New("login required"))
			} else {
		...
		if !user.IsStaff && !user.IsSuperUser {
			voiceSculptor.AbortWithJSONError(ctx, http.StatusForbidden, errors.New("forbidden"))
			return
		}
		ctx.Next()
	}
...
func RegisterAdmins(r *gin.RouterGroup, db *gorm.DB, objs []AdminObject) {
	...
	r.GET("/*filepath", func(ctx *gin.Context) {
		staticAssets := ctx.MustGet(constants.AssetsField).(*voiceSculptor.CombineEmbedFS)
		...
		if name == "/" {
			...
			ctx.HTML(http.StatusOK, "app.html", gin.H{
				"Scripts":   jsFiles,
				"Styles":    cssFiles,
				"Dashboard": util.GetValue(db, KEY_ADMIN_DASHBOARD),
...
func HandleAdminJson(c *gin.Context, objects []*AdminObject, buildContext AdminBuildContext) {
	...
	for _, obj := range objects {
		...
		db := voiceSculptor.GetDbConnection(c, obj.GetDB, false)
		val := *obj
		val.BuildPermissions(db, CurrentUser(c))
		viewObjects = append(viewObjects, val)
	}

	siteCtx := voiceSculptor.GetRenderPageContext(c)
	if buildContext != nil {
...
func (obj *AdminObject) RegisterAdmin(r gin.IRoutes) {
	r = r.Use(func(ctx *gin.Context) {
		if obj.AccessCheck != nil {
			...
			if err != nil {
				voiceSculptor.AbortWithJSONError(ctx, http.StatusForbidden, err)
				return
			}
		}
		ctx.Next()
...
func (obj *AdminObject) MarshalOne(c *gin.Context, val interface{}) (map[string]any, error) {
	...
	if obj.ViewOnSite != nil {
		result["_adminExtra"] = map[string]any{
			"viewOnSite": obj.ViewOnSite(voiceSculptor.GetDbConnection(c, obj.GetDB, false), c, val),
		}
	}
...
func (obj *AdminObject) handleGetOne(c *gin.Context) {
	...
	if len(keys) <= 0 {
		voiceSculptor.AbortWithJSONError(c, http.StatusBadRequest, errors.New("invalid primary key"))
		return
	}
	...
	if result.Error != nil {
		voiceSculptor.AbortWithJSONError(c, http.StatusInternalServerError, result.Error)
		return
	}
	...
	if obj.BeforeRender != nil {
		...
		if err != nil {
			voiceSculptor.AbortWithJSONError(c, http.StatusInternalServerError, err)
			return
		}
		if rr != nil {
...
func (obj *AdminObject) QueryObjects(session *gorm.DB, form *voiceSculptor.QueryForm, ctx *gin.Context) (r AdminQueryResult, err error) {
	for _, v := range form.Filters {
		if q := v.GetQuery(); q != "" {
			if v.Op == voiceSculptor.FilterOpLike {
			...
			} else if v.Op == voiceSculptor.FilterOpBetween {
	...
	var orders []voiceSculptor.Order
	...
	for i := 0; i < vals.Elem().Len(); i++ {
		...
		if obj.BeforeRender != nil {
			db := voiceSculptor.GetDbConnection(ctx, obj.GetDB, false)
...
func (obj *AdminObject) handleQueryOrGetOne(c *gin.Context) {
	...
	db, form, err := voiceSculptor.DefaultPrepareQuery(voiceSculptor.GetDbConnection(c, obj.GetDB, false), c)
	if err != nil {
...
func (obj *AdminObject) handleCreate(c *gin.Context) {
	...
	db := voiceSculptor.GetDbConnection(c, obj.GetDB, true)
...
func (obj *AdminObject) handleUpdate(c *gin.Context) {
	...
	if err != nil {
		voiceSculptor.AbortWithJSONError(c, http.StatusNotFound, errors.New("not found"))
		return
	}
...
func (obj *AdminObject) handleDelete(c *gin.Context) {
	...
	if r.Error != nil {
		if errors.Is(r.Error, gorm.ErrRecordNotFound) {
			...
			voiceSculptor.AbortWithJSONError(c, http.StatusInternalServerError, r.Error)
		}
		return
	}
