...
func GetRenderPageContext(c *gin.Context) map[string]any {
	...
	util.LoadAutoloads(db)
	loginNext := c.Query("next")
	if loginNext == "" {
		loginNext = util.GetValue(db, constants.KEY_SITE_LOGIN_NEXT)
	}
	...
	return map[string]any{
		...
		"Site": map[string]any{
			...
			"Admin":                util.GetValue(db, constants.KEY_SITE_ADMIN),
			"Keywords":             util.GetValue(db, constants.KEY_SITE_KEYWORDS),
			"Description":          util.GetValue(db, constants.KEY_SITE_DESCRIPTION),
			"GA":                   util.GetValue(db, constants.KEY_SITE_GA),
			"LogoUrl":              util.GetValue(db, constants.KEY_SITE_LOGO_URL),
			"FaviconUrl":           util.GetValue(db, constants.KEY_SITE_FAVICON_URL),
			"TermsUrl":             util.GetValue(db, constants.KEY_SITE_TERMS_URL),
			"PrivacyUrl":           util.GetValue(db, constants.KEY_SITE_PRIVACY_URL),
			"SigninUrl":            util.GetValue(db, constants.KEY_SITE_SIGNIN_URL),
			"SignupUrl":            util.GetValue(db, constants.KEY_SITE_SIGNUP_URL),
			"LogoutUrl":            util.GetValue(db, constants.KEY_SITE_LOGOUT_URL),
			"ResetPasswordUrl":     util.GetValue(db, constants.KEY_SITE_RESET_PASSWORD_URL),
			"SigninApi":            util.GetValue(db, constants.KEY_SITE_SIGNIN_API),
			"SignupApi":            util.GetValue(db, constants.KEY_SITE_SIGNUP_API),
			"ResetPasswordDoneApi": util.GetValue(db, constants.KEY_SITE_RESET_PASSWORD_DONE_API),
			"UserIdType":           util.GetValue(db, constants.KEY_SITE_USER_ID_TYPE),
		},
	}
}
